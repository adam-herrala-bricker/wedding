...
const HighlightView = () => {
  ...
  const adjoiningImage = (direction) => {
    ...
    // direction = left (doesn't move if at beginning)
  ...
  // uses same direction encoding as the arrow handler,
  // since it shares a helper function
  ...
  // Note: Sending the entry token in the query itself.
  // Can't imagine this is the best idea, but all the ways of
  // authenticating the requests in the img component
